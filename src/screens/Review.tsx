...
import { editBill, friendlyError } from "../ai";
...
export function Review() {
  ...
  const applyAi = async (instruction: string) => {
    if (!apiKey) {
      setAiError("Add your API key in Settings to use voice/text edits.");
      return;
    }
    setAiBusy(true);
    setAiError(null);
    try {
      const { bill: edited, changedIds } = await editBill(apiKey, bill, instruction);
      setDraft(edited);
    ...
    <div className="mx-auto flex min-h-dvh max-w-md flex-col">
      ...
      <div className="flex-1 space-y-4 px-5 py-4 pb-44">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="grid size-9 shrink-0 place-items-center rounded-lg text-dim active:text-coral"
          ...
          {bill.totalCents > 0 && (
            <div
              className={`rounded-lg px-3 py-2 text-center text-sm font-medium ${
                delta === 0 ? "bg-mint/10 text-mint" : "bg-coral/10 text-coral"
              }`}
            >
              {delta === 0
                ? "Balances to $0.00 ✓"
        ...
        <div className="flex items-center justify-between rounded-2xl border border-line bg-card p-4">
          ...
          <Stepper value={bill.people} onChange={(v) => patch({ people: v })} min={1} />
        </div>

        {aiError && <div className="rounded-xl border border-coral/40 bg-coral/10 p-3 text-sm text-coral">{aiError}</div>}
      </div>
