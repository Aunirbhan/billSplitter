  ...
  return (
    <div className="flex items-center gap-2">
      {hasMic && (
        <button
          ...
          className={`grid size-12 shrink-0 place-items-center rounded-full border transition-colors ${
            recording ? "border-coral bg-coral/20 text-coral" : "border-line bg-card-hi text-dim active:text-ink"
          }`}
        >
          {recording ? (
            <span className="size-3.5 rounded-sm bg-coral" />
          ) : (
      ...
      <input
        ...
        className={`min-w-0 flex-1 rounded-full border border-line bg-card-hi px-4 py-3 text-ink placeholder:text-dim/60 focus:border-amber focus:outline-none ${busy ? "shimmer" : ""}`}
      />
      <button
        onClick={submit}
        disabled={busy || !text.trim()}
        aria-label="Apply edit"
        className="grid size-12 shrink-0 place-items-center rounded-full bg-amber text-black disabled:opacity-30"
      >
        {busy ? (
